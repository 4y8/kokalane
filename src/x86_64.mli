...
val register8: [`Q] register -> [`B] register
  (** accès au registre des 8 bits faibles d'un registre 64 bits *)
...
val aligned_call_wrapper: f:string -> newf:string -> text
  (** construit une fonction assembleur `newf` qui appelle `f` après avoir
      correctement aligné la pile (sur un multiple de 16).
      Ainsi, `aligned_call "malloc" "malloc_"` ajoute au code assembleur
      une fonction `malloc_` qu'on peut appeler en tout endroit de notre
      code sans se soucier d'alignement. *)
